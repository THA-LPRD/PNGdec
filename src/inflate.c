...
z_streamp strm;
{
    ...
    state->sane = 1;
    state->back = -1;
    PNGDEC_TRACEv((stderr, "inflate: reset\n"));
    return Z_OK;
}
...
int stream_size;
{
    ...
    //    PNGDEC_TRACEv((stderr, "inflate: allocated\n"));
...
int check_crc;
{
    ...
    for (;;)
        ...
        case HEAD:
            ...
            state->dmax = 1U << len;
            PNGDEC_TRACEv((stderr, "inflate:   zlib header ok\n"));
            if (check_crc)
            {
        ...
        case TYPEDO:
            ...
            {
            case 0: /* stored block */
                PNGDEC_TRACEv((stderr, "inflate:     stored block%s\n",
                               state->last ? " (last)" : ""));
                state->mode = STORED;
                break;
            case 1: /* fixed block */
                fixedtables(state);
                PNGDEC_TRACEv((stderr, "inflate:     fixed codes block%s\n",
                               state->last ? " (last)" : ""));
            ...
            case 2: /* dynamic block */
                PNGDEC_TRACEv((stderr, "inflate:     dynamic codes block%s\n",
                               state->last ? " (last)" : ""));
                state->mode = TABLE;
                break;
            case 3:
        ...
        case STORED:
            ...
            PNGDEC_TRACEv((stderr, "inflate:       stored length %u\n",
                           state->length));
            INITBITS();
            state->mode = COPY_;
            if (flush == Z_TREES)
        ...
        case COPY:
            ...
            PNGDEC_TRACEv((stderr, "inflate:       stored end\n"));
            state->mode = TYPE;
            break;
        case TABLE:
            NEEDBITS(14);
...
#endif
            PNGDEC_TRACEv((stderr, "inflate:       table sizes ok\n"));
            state->have = 0;
            state->mode = LENLENS;
        case LENLENS:
            ...
            PNGDEC_TRACEv((stderr, "inflate:       code lengths ok\n"));
            state->have = 0;
            state->mode = CODELENS;
        case CODELENS:
            ...
            PNGDEC_TRACEv((stderr, "inflate:       codes ok\n"));
            state->mode = LEN_;
            if (flush == Z_TREES)
                goto inf_leave;
        case LEN_:
        ...
        case MATCH:
            ...
            { /* copy from window */
                ...
                if (copy > state->whave)
                {
                    ...
                    PNGDEC_TRACE((stderr, "inflate.c too far\n"));
                    copy -= state->whave;
        ...
        case CHECK:
            if (state->wrap)
            {
                ...
                INITBITS();
                PNGDEC_TRACEv((stderr, "inflate:   check matches trailer\n"));
            }
#ifdef GUNZIP
            state->mode = LENGTH;
        case LENGTH:
            if (state->wrap && state->flags)
            {
                ...
                INITBITS();
                PNGDEC_TRACEv((stderr, "inflate:   length matches trailer\n"));
            }
#endif
            state->mode = DONE;
        case DONE:
...
z_streamp strm;
{
    ...
    strm->state = Z_NULL;
    PNGDEC_TRACEv((stderr, "inflate: end\n"));
    return Z_OK;
}
...
uInt dictLength;
{
    ...
    state->havedict = 1;
    PNGDEC_TRACEv((stderr, "inflate:   dictionary set\n"));
    return Z_OK;
}
